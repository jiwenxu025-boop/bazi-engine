...
# open_phase 逃逸阀（v0.16）
当数据包中出现 `_signal_conflict_list` 时，说明引擎检测到了严重矛盾的信号对（如"表达欲高"同时"内敛度高"）。
遇到矛盾信号时：
- **不要硬给单一结论**。在对应维度段落的末尾，用"需要注意的是"开头指出矛盾，展示两种可能的解读。
- **不要用"可能是A也可能是B"这种废话搪塞**。要具体说明：在什么场景下表现为A，在什么场景下表现为B。
- 如果整盘的核心矛盾（全局诊断中的）属于"难以融合"级别，可以在结尾"立刻能做的事"之前加一个 `### 另一种可能` 小节，简要描述另一种性格走向。这个可选项仅在矛盾极其深刻时使用。
- **全局最高指令的优先级永远高于矛盾融合**——如果病药检测给出了明确的全局指令，以它为准，矛盾作为细节补充而非推翻全局。"""
...
def build_fusion_data_package(pr_dict: dict, family_dict: dict | None = None,
    ...
    # ── 信号矛盾检测（v0.16: open_phase 逃逸阀）──
    if trait_signals:
        conflicts = []
        # 检测同一维度内的高-高矛盾对
        _conflict_pairs = [
            (["表达欲", "分享倾向", "社交能量"], ["内敛度", "独处需求", "拘谨度"]),
            (["果断度", "冲动性", "冒险倾向"], ["审慎度", "保守倾向"]),
            (["竞争欲"], ["合作倾向", "不喜冲突"]),
        ]
        for dim in ["社交", "感情", "内心", "决策", "事业", "财富观"]:
            raw = trait_signals.get(dim, {})
            for high_group, low_group in _conflict_pairs:
                high_hit = None
                low_hit = None
                for k in high_group:
                    if k in raw and isinstance(raw[k], (int, float)) and raw[k] >= 7:
                        high_hit = f"{k}={raw[k]}"
                        break
                for k in low_group:
                    if k in raw and isinstance(raw[k], (int, float)) and raw[k] >= 7:
                ...
                if high_hit and low_hit:
                    conflicts.append({
                        "维度": dim,
                        "矛盾对": f"{high_hit} ⇄ {low_hit}",
                        "说明": f"在{dim}维度中同时存在{high_hit.split('=')[0]}偏高和{low_hit.split('=')[0]}偏高——这暗示同一个人在不同情境下可能表现出截然相反的倾向"
                    })
        if conflicts:
            package["_signal_conflict_list"] = conflicts[:3]  # 最多3对
